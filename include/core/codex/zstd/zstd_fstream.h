...
struct zstd_ifstream_base {
    zstd_ifstream_base(const string& filename) : ifs_(filename) { }
    std::ifstream ifs_;
};
...
public:
    zstd_ifstream(const string& filename, size_t n = 0)
...
struct zstd_ofstream_base {
    zstd_ofstream_base(const string& filename) : ofs_(filename) { }
    std::ofstream ofs_;
};
